...
export const PUT = async (req: Request): Promise<any> => {
  ...
  let vote = await UpVoteModel.findOne({ user, forum: body?._id });
  if (vote) {
    return NextResponse.json(
  ...
  vote = await DownVoteModel.findOne({ user, forum: body?._id });
  const answer = await AnswerModel.findById(body?._id);
  const question = await ForumModel.findById(body?._id);
  if (vote) {
    await DownVoteModel.findOneAndDelete({ user, forum: body?._id });
  } else {
    await UpVoteModel.create({ user, forum: body?._id });
  }
  if (Boolean(answer)) {
